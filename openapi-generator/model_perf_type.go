...
// PerfType the model 'PerfType'
type PerfType string

// List of PerfType
const (
	PERFTYPE_ULTRA_BULLET PerfType = "ultraBullet"
	PERFTYPE_BULLET PerfType = "bullet"
	PERFTYPE_BLITZ PerfType = "blitz"
	PERFTYPE_RAPID PerfType = "rapid"
	PERFTYPE_CLASSICAL PerfType = "classical"
	PERFTYPE_CORRESPONDENCE PerfType = "correspondence"
	PERFTYPE_CHESS960 PerfType = "chess960"
	PERFTYPE_CRAZYHOUSE PerfType = "crazyhouse"
	PERFTYPE_ANTICHESS PerfType = "antichess"
	PERFTYPE_ATOMIC PerfType = "atomic"
	PERFTYPE_HORDE PerfType = "horde"
	PERFTYPE_KING_OF_THE_HILL PerfType = "kingOfTheHill"
	PERFTYPE_RACING_KINGS PerfType = "racingKings"
	PERFTYPE_THREE_CHECK PerfType = "threeCheck"
	PERFTYPE_UNKNOWN_DEFAULT_OPEN_API PerfType = "unknown_default_open_api"
)

// All allowed values of PerfType enum
var AllowedPerfTypeEnumValues = []PerfType{
	"ultraBullet",
	"bullet",
	"blitz",
	"rapid",
...
func (v *PerfType) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := PerfType(value)
	for _, existing := range AllowedPerfTypeEnumValues {
	...
	*v = PERFTYPE_UNKNOWN_DEFAULT_OPEN_API
	return nil
}

// NewPerfTypeFromValue returns a pointer to a valid PerfType
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewPerfTypeFromValue(v string) (*PerfType, error) {
	ev := PerfType(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for PerfType: valid values are %v", v, AllowedPerfTypeEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v PerfType) IsValid() bool {
	for _, existing := range AllowedPerfTypeEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to PerfType value
func (v PerfType) Ptr() *PerfType {
	return &v
}
